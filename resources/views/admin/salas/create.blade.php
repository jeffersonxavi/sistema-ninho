...
<div class="flex items-center gap-4">
    ...
    <div>
        <h2 class="font-black text-2xl text-gray-800 tracking-tight leading-none">Nova Sala</h2>
        <p class="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-500 mt-1">Gestão de Espaços</p>
    </div>
</div>
@endsection
...
<div class="py-12 bg-gray-50/50 min-h-screen">
    <div class="max-w-3xl mx-auto px-4 lg:px-8">
        <form method="POST" action="{{ route('admin.salas.store') }}" class="space-y-8">
            @csrf
            ...
            <div class="bg-white rounded-[2.5rem] border border-gray-100 p-8 md:p-10 shadow-sm">
                <div class="space-y-8">
                    {{-- NOME DA SALA --}}
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Nome ou Número da Sala</label>
                        <input type="text" name="nome" value="{{ old('nome') }}" required placeholder="Ex: Sala de Artes 01"
                    ...
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 block mb-4">Vincular Professores (Opcional)</label>
                            ...
                            @forelse ($professores as $prof)
                                ...
                                <label class="flex items-center p-4 rounded-2xl border-2 transition-all cursor-pointer {{ $selected ? 'border-indigo-600 bg-indigo-50/30' : 'border-gray-50 bg-gray-50/50 hover:border-gray-200' }}">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <p class="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">Clique para Selecionar</p>
                                    </div>
                                </label>
                            @empty
                                <div class="col-span-full py-8 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-100">
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Nenhum docente disponível</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
