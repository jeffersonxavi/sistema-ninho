...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (Auth::user()->isAdmin())
                        ...
                        {{-- LINK UNIFICADO: Turmas e Salas --}}
                        <x-nav-link :href="route('admin.turmas.index')" 
                            :active="request()->routeIs('admin.turmas.*') || request()->routeIs('admin.salas.*')">
                            Turmas & Salas
                        </x-nav-link>
                    ...
                    @if (Auth::user()->isStaff())
                        {{-- Se o Staff também usar a view unificada, aponte para turmas.index --}}
                        <x-nav-link :href="route('staff.salas.index')" 
                            :active="request()->routeIs('staff.salas.*') || request()->routeIs('staff.turmas.*')">
                            Turmas & Salas
                        </x-nav-link>
            ...
            {{-- Dropdown de Usuário (Profile/Logout) --}}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if (Auth::user()->isAdmin())
                <x-responsive-nav-link :href="route('admin.turmas.index')" :active="request()->routeIs('admin.turmas.*') || request()->routeIs('admin.salas.*')">
                    Turmas & Salas
                </x-responsive-nav-link>
                {{-- Adicione os outros links aqui se desejar no mobile --}}
            @endif
        </div>
